    ...
    def default_get(self, fields):
        ...
        if picking:
            ...
            for move in picking.move_lines:
                ...
                # tungnt start 2
                fory_price_unit = 0.0
                fory_price_unit_with_tax = 0.0
                for pack_operation_product in picking.pack_operation_product_ids:
                    if(pack_operation_product.product_id.id == move.product_id.id):
                        fory_price_unit = pack_operation_product.fory_price_unit
                        fory_price_unit_with_tax = pack_operation_product.fory_price_unit_with_tax

                new_vals = {
                    'product_id': move.product_id.id,
                    'quantity': quantity,
                    'move_id': move.id,
                    'fory_price_unit' : fory_price_unit,
                    'fory_price_unit_with_tax' : fory_price_unit_with_tax
                }
                product_return_moves.append((0, 0, new_vals))
                # tungnt end 2
    ...
    def _create_returns(self):
        ...
        for return_line in self.product_return_moves:
            ...
            if new_qty:
                ...
                # tungnt start 2
                return_amount = return_line.fory_price_unit_with_tax * new_qty
                total_return_amount += return_amount
                # tungnt end 2

        # tungnt start 2
        if(returned_lines > 0):
            # stock_quants = return_moves.quant_ids
            # total_return_amount = 0
            # for quant in stock_quants:
            #     return_amount = quant._compute_inventory_value()
            #     return_amount = quant.cost * quant.qty
            #     total_return_amount += return_amount

            sale_order = self.env['sale.order'].sudo().search([('name','=',return_moves[0].origin )], limit = 1)
            if(len(sale_order.ids) > 0):
                # sale_order.changed_point
                card = self.env['corefory.loyalty.card'].sudo()._get_card(sale_order.partner_id.id)
                if(sale_order.added_money <= total_return_amount):
                    sale_order.return_loyalty_point(sale_order.changed_point,sale_order.added_point)
                else:
                    card_change_history = self.env['corefory.loyalty.card.history'].sudo().search([('origin','=',sale_order.name), ('money_to_point','>' ,0)], limit = 1);
                    ...
                    if(sale_order.apply_loyalty_card and sale_order.changed_point > 0):
                        is_converted, changed_point = self.env['corefory.loyalty.card'].sudo().convert_amount_to_point_base_with_param( total_return_amount,card_change_history.point_to_money)
                        if(changed_point >= sale_order.changed_point):
                            changed_point = sale_order.changed_point
                    else:
                        changed_point = 0.0
                    sale_order.return_loyalty_point(changed_point,added_point)
