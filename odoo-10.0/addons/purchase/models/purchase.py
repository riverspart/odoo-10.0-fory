    ...
    def _get_stock_move_price_unit(self):
        ...
        if order.currency_id != order.company_id.currency_id:
            ...
            if(len(self.move_ids.ids) > 0) :
                posted_date =self.move_ids[0].temp_posted_date

                for stock_move in self.move_ids:
                    stock_move.update({'price_unit_before': price_unit})
            else:
                posted_date = fields.Datetime.now()

            price_unit = order.currency_id.compute_2(price_unit, order.company_id.currency_id, round=False, date= posted_date)

            # tungnt end

            # tungnt origin start
            # price_unit = order.currency_id.compute_2(price_unit, order.company_id.currency_id, round=False)
            # tungnt origin end
        return price_unit
