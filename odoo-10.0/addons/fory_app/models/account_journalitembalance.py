...
class Account_JournalItemBalance(models.Model):

    _name= "account.journalitembalance"
    ...
    def create(self, vals):
        ...
        excel = super(Account_JournalItemBalance, self).create(vals)
        return excel
        # return {
        #     'view_mode': 'form',
        #     'res_id': export_id.id,
    ...
    def create_excel_report(self, date_from, date_to, company_name, company_address, locations):
        ...
        worksheet = workbook.add_sheet(u'Bảng cân đối phát sinh', cell_overwrite_ok=True)
        ...
        str_date_from = _(u"Từ ngày %s đến %s") % (date_from.strftime("%d/%m/%Y %H:%M:%S"), date_to.strftime("%d/%m/%Y %H:%M:%S"))
        row = row + 1
            ...
            for location in locations:
                ...
                if products:
                    ...
                    self._cr.execute("""SELECT product_id AS id, 
                                          ...
                                          GROUP BY product_id""",
                                     (location.id, date_from, tuple(pids)))
                    previous_data = self._cr.dictfetchall()
                    ...
                    col = col + 7
                    worksheet.write(row, col, xlwt.Formula(_('SUM(H%s:H%s)') % (row + 2, row + 1 + p_len)), style_quantity_bold)
                    col = col + 1
                    worksheet.write(row, col, xlwt.Formula(_('SUM(I%s:I%s)') % (row + 2, row + 1 + p_len)), style_currency_bold)
                    col = col + 1
                    worksheet.write(row, col, xlwt.Formula(_('SUM(J%s:J%s)') % (row + 2, row + 1 + p_len)), style_quantity_bold)
                    col = col + 1
                    worksheet.write(row, col, xlwt.Formula(_('SUM(K%s:K%s)') % (row + 2, row + 1 + p_len)), style_currency_bold)
                    col = col + 1
                    worksheet.write(row, col, xlwt.Formula(_('SUM(L%s:L%s)') % (row + 2, row + 1 + p_len)), style_quantity_bold)
                    col = col + 1
                    worksheet.write(row, col, xlwt.Formula(_('SUM(M%s:M%s)') % (row + 2, row + 1 + p_len)), style_currency_bold)
                    col = col + 1
                    worksheet.write(row, col, xlwt.Formula(_('SUM(N%s:N%s)') % (row + 2, row + 1 + p_len)), style_quantity_bold)
                    col = col + 1
                    worksheet.write(row, col, xlwt.Formula(_('SUM(O%s:O%s)') % (row + 2, row + 1 + p_len)), style_currency_bold)
                    ...
                    for p in products:
                        ...
                        col += 1
                        worksheet.write(row, col, p['quantity_import'], style_quantity)
                        col += 1
                        worksheet.write(row, col, p['sum_cost_import'], style_currency)
                        col += 1
                        worksheet.write(row, col, p['quantity_export'], style_quantity)
                        col += 1
                        worksheet.write(row, col, p['sum_cost_export'], style_currency)
                        col += 1
        ...
        col = col + 7
        worksheet.write(row, col, xlwt.Formula(_('SUM(H%s:H%s)/2') % (9, row)), style_quantity_bold)
        col = col + 1
        worksheet.write(row, col, xlwt.Formula(_('SUM(I%s:I%s)/2') % (9, row)), style_currency_bold)
        col = col + 1
        worksheet.write(row, col, xlwt.Formula(_('SUM(J%s:J%s)/2') % (9, row)), style_quantity_bold)
        col = col + 1
        worksheet.write(row, col, xlwt.Formula(_('SUM(K%s:K%s)/2') % (9, row)), style_currency_bold)
        col = col + 1
        worksheet.write(row, col, xlwt.Formula(_('SUM(L%s:L%s)/2') % (9, row)), style_quantity_bold)
        col = col + 1
        worksheet.write(row, col, xlwt.Formula(_('SUM(M%s:M%s)/2') % (9, row)), style_currency_bold)
        col = col + 1
        worksheet.write(row, col, xlwt.Formula(_('SUM(N%s:N%s)/2') % (9, row)), style_quantity_bold)
        col = col + 1
        worksheet.write(row, col, xlwt.Formula(_('SUM(O%s:O%s)/2') % (9, row)), style_currency_bold)
    ...
    def write(self, vals):
        ...
        excel = super(Account_JournalItemBalance, self).write(vals)
