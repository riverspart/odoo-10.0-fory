...
class LandedCost(models.Model):
            ...
            for line in cost.valuation_adjustment_lines.filtered(lambda line: line.move_id):
                ...
                if(line.product_id.cost_method == 'average'):
                    if(line.product_id.qty_available != 0):
                        new_std_price = (line.product_id.qty_available * line.product_id.standard_price + line.additional_landed_cost) / line.product_id.qty_available
                        line.product_id.with_context(force_company=move.company_id.id).sudo().write({'standard_price': new_std_price})
                # tungnt end
    ...
    def get_valuation_lines(self):
        ...
        for move in self.mapped('picking_ids').mapped('move_lines'):
            ...
            # tungnt start
            if move.product_id.valuation == 'real_time' and (move.product_id.cost_method == 'real' or move.product_id.cost_method == 'average') :
                vals = {
                    'product_id': move.product_id.id,
                    'move_id': move.id,
                    'quantity': move.product_qty,
