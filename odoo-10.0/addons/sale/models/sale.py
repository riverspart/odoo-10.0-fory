        ...
        for order in self:
            ...
            for picking in order.picking_ids:
                for stock_pack_operation in picking.pack_operation_product_ids:
                    for order_product in order.order_line:
                        if stock_pack_operation.product_id.id == order_product.product_id.id:
                            stock_pack_operation.update({
                                'fory_price_unit':order_product.price_unit,
                                'fory_price_unit_with_tax': (order_product.price_subtotal + order_product.price_tax)/ order_product.product_uom_qty
                            })
            #  tungnt end 2
