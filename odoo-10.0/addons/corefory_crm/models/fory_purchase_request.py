...
class CoreforyPurchaseRequest(models.Model):

    _name = 'corefory.purchase.request'
    _description = 'Fory Purchase Request'
    ...
    def _get_default_name(self):
        return self.env['ir.sequence'].next_by_code('corefory.purchase.request')
    ...
    line_ids = fields.One2many('corefory.purchase.request.line', 'request_id',
    ...
    department_id = fields.Many2one('hr.department', string='Department', compute='_compute_department', store=True, required=True,)
    ...
    def _compute_can_manager_approved(self):
        ...
        if (self.state == 'leader_approved' and current_user.has_group('corefory_crm.group_corefory_purchase_request_manager')):
    ...
    def create(self, vals):
        request = super(CoreforyPurchaseRequest, self).create(vals)
    ...
    @api.multi
    def write(self, vals):
        res = super(CoreforyPurchaseRequest, self).write(vals)
        for request in self:
            if vals.get('assigned_to'):
...
class CoreforyPurchaseRequestLine(models.Model):

    _name = "corefory.purchase.request.line"
    _description = "Corefory Purchase Request Line"
    _inherit = ['mail.thread', 'ir.needaction_mixin']
    ...
    request_id = fields.Many2one('corefory.purchase.request',
                                 'Purchase Request',
    ...
    @api.multi
    def write(self, vals):
        res = super(CoreforyPurchaseRequestLine, self).write(vals)
